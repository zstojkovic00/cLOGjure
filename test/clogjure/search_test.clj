...
(facts "by-and-words - searches log with AND logic"
       (fact "single word search returns matching lines"
             (let [matching-lines (search/by-and-words ["zeljko"] test-log-path nil nil)]
       ...
       (fact "multiple words search returns matching lines"
             (let [matching-lines (search/by-and-words ["AbstractiveVersionControlSystemManagerApplicationKt" "process"] test-log-path nil nil)]
               matching-lines => vector?
       ...
       (fact "multiple words search with date filter"
             (let [matching-lines (search/by-and-words ["AbstractiveVersionControlSystemManagerApplicationKt" "process"] test-log-path
                                                       (util/to-unix-timestamp "2025-12-05T22:15:00.338+01:00") (util/to-unix-timestamp "2025-12-05T22:20:01.338+01:00"))]
               matching-lines => vector?
       ...
       (fact "multiple words search with date filter out of range"
             (let [matching-lines (search/by-and-words ["AbstractiveVersionControlSystemManagerApplicationKt" "process"] test-log-path
                                                       (util/to-unix-timestamp "2025-12-05T22:30:00.338+01:00") (util/to-unix-timestamp "2025-12-05T22:35:01.338+01:00"))]
               matching-lines => nil
...
(facts "by-prefix - searches log by prefix"
       (fact "single word search returns matching lines"
             (let [matching-lines (search/by-prefix-words ["zelj"] test-log-path nil nil)]
               matching-lines => vector?
               (count matching-lines) => 2                  ;; prefix zelj appears in two lines [8, 10]
               (get-in matching-lines [0 :line]) => "2026-01-19T10:02:00 INFO zeljko je kralj"
               (get-in matching-lines [1 :line]) => "2026-01-19T10:00:00 INFO zeljko")

             (let [matching-lines (search/by-prefix-words ["rad"] test-log-path nil nil)]
               matching-lines => vector?
             ...
             (let [matching-lines (search/by-prefix-words ["info"] test-log-path nil nil)]
               matching-lines => vector?
               (count matching-lines) => 8))                ;; prefix info appears in eight lines

       (fact "non-existent prefix returns nil"
             (search/by-prefix-words ["asd"] test-log-path nil nil) => nil))

(facts "by-or-words - searches log with OR logic"
       (fact "single word search returns matching lines"
             (let [matching-lines (search/by-or-words ["zeljko"] test-log-path nil nil)]
               matching-lines => vector?
               (count matching-lines) => 2))

       (fact "multiple words OR returns lines containing any word"
             (let [matching-lines (search/by-or-words ["zeljko" "error"] test-log-path nil nil)]
               matching-lines => vector?
       ...
       (fact "non-existent words returns nil"
             (search/by-or-words ["asd" "dsa"] test-log-path nil nil) => nil))
